...
const { phoneBlacklistModule } = await import("./index.ts");
...
function buildApp(overrides: Partial<FakeService>) {
  ...
  return new Elysia().use(
    phoneBlacklistModule({ phonePoliciesService: full as unknown as PhonePoliciesService })
  );
}
...
describe("POST /admin/moderation/blacklist", () => {
...
describe("GET /admin/moderation/blacklist", () => {
  it("401 sem x-api-key", async () => {
    const res = await makeRequest(app, "/admin/moderation/blacklist", {}, null);
    expect(res.status).toBe(401);
  });
  ...
  it("passa kind=blacklist para o service", async () => {
    ...
    await makeRequest(app, "/admin/moderation/blacklist");
    expect(capturedKind).toBe("blacklist");
  });
});

describe("GET /admin/moderation/blacklist/:id", () => {
  it("401 sem x-api-key", async () => {
    const res = await makeRequest(app, `/admin/moderation/blacklist/${VALID_ID}`, {}, null);
    expect(res.status).toBe(401);
  });

  it("200 retorna entry da blacklist", async () => {
    const res = await makeRequest(app, `/admin/moderation/blacklist/${VALID_ID}`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as OkResp<PhonePolicyView>;
  ...
  it("404 quando id pertence a entrada bypass (cross-kind)", async () => {
    app = buildApp({ get: async () => makeView({ kind: "bypass" }) });
    const res = await makeRequest(app, `/admin/moderation/blacklist/${VALID_ID}`);
    expect(res.status).toBe(404);
  });
});

describe("DELETE /admin/moderation/blacklist/:id", () => {
  it("401 sem x-api-key", async () => {
    const res = await makeRequest(
      app,
      `/admin/moderation/blacklist/${VALID_ID}`,
      { method: "DELETE" },
      null
    );
    expect(res.status).toBe(401);
